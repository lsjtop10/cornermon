...
structs = [
    ...
    'DeviceRegistration', 'Group', 'Track', 'FacilitatorSession', 'Message', 'Camp'
]
...
def process_file(path):
    ...
    for struct in structs:
        # We need to replace `domain.Struct{` with `domain.NewStructValFromProps(domain.StructProps{`
        # and `&domain.Struct{` with `domain.NewStructFromProps(domain.StructProps{`
        
        # 1. &domain.Struct{
        pattern_ptr = r'&domain\.' + struct + r'\{'
        pos = 0
        while True:
        ...
        # 2. domain.Struct{ (not preceded by &)
        pattern_val = r'(?<!&)\bdomain\.' + struct + r'\{'
        pos = 0
        while True:
