...
func (r *pgAnnouncementRepository) Save(ctx context.Context, a *domain.Announcement) error {
	err := r.queries(ctx).SaveAnnouncement(ctx, db.SaveAnnouncementParams{ID: string(a.ID), CampID: pgtype.Text{String: string(a.CampID), Valid: true}, SenderRole: string(a.SenderRole), Content: a.Content, SentAt: pgtype.Timestamptz{Time: a.SentAt, Valid: true}})
	if err != nil {
...
func (r *pgAnnouncementRepository) ListNoticeByCamp(ctx context.Context, campID domain.CampID) ([]*domain.Announcement, error) {
	rows, err := r.queries(ctx).ListAnnouncementsByCamp(ctx, pgtype.Text{String: string(campID), Valid: true})
	if err != nil {
