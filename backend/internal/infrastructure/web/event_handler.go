...
// @Description  트랙 진행자용 best-effort 변경 알림 스트림입니다. 각 event의 data는 SSENotification JSON이며 예시는 {"event":"track_updated","scope":{"kind":"track","trackId":"track-id"}} 입니다. 일반 이벤트에는 상태 스냅샷이 없으므로 REST API로 최신 상태를 조회합니다. 단, `camp_ended`는 terminal event이므로 REST 재조회 없이 로컬 진행자 세션·기기 등록을 삭제하고 등록 화면으로 이동합니다. 서버는 유실된 메시지를 저장·재전송하지 않으며, SSE 유실은 GET /device-registrations/me의 REVOKED/ENDED 상태로 복구합니다.
// @Tags         F. Events (SSE)
// @Security     TrackAuth
// @Produce      text/event-stream
