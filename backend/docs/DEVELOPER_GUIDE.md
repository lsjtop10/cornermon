...
- 파일명은 `{YYYYMMDDHHMMSS}_{설명}.up.sql` / `{YYYYMMDDHHMMSS}_{설명}.down.sql` 쌍으로 만든다
  (예: `20260724143000_add_track_note.up.sql`). 마이그레이션 작성 시각 기준 타임스탬프를 쓰면
  브랜치가 갈렸다 나중에 합쳐져도 번호가 충돌하지 않는다. 반드시 오름차순 정렬되어야 하며, sqlc가
