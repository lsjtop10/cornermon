...
| **P0** | UC-2: `schema.sql` → 버전별 마이그레이션 파일 전환 | 기존 스키마 전체를 `000001_init_schema.up/down.sql`로 이관, 기존 임시 마이그레이션 흡수 | **베이스라인 확립** |
...
| 순서 | 작업 | 파일 |
| --- | --- | --- |
| A-1 | 기존 `schema.sql` 전체 내용 + `20260721_add_corner_deleted_at.sql`(soft-delete 컬럼/인덱스)을 흡수해 `000001_init_schema.up.sql` 작성 | `db/migrations/000001_init_schema.up.sql` |
| A-2 | 전체 테이블을 참조 관계 역순으로 DROP하는 down 마이그레이션 작성 | `db/migrations/000001_init_schema.down.sql` |
...
- [x] UC-2: `db/migrations/000001_init_schema.up.sql` 적용 결과가 기존 `schema.sql`과 동일한
