...
0. plan.md 최상단에 `Issue: #번호`와 원 요구사항(무엇을 왜 요청받았는지 1~3줄)을 남깁니다. 이슈가 아직 없으면 먼저 생성합니다.
1. 계획을 작성하기 전 반드시 코드를 깊이 이해하는 과정을 거칩니다.
   1. 특히 새로 작성할 코드와 유사한 코드를 집중적으로 이해합니다.
2. 사용자의 요구사항 중 모호한 부분이 있거나 기존 코드베이스와 충돌하는 경우 사용자에게 먼저 질문을 해서 반드시 모호함을 해결합니다. 
3. 특별한 이유가 없으면 이미 존재하는 코드의 관례를 따릅니다.
4. 계획 단계에서 readme, 개발자 가이드라인을 반드시 참조합니다.
5. 변경 사항을 작성할 때 기존 코드베이스와 일관성을 지켜야 합니다. 이미 있는 로직을 중복 구현하거나 기존 코드를 함부로 수정해서 되던 기능에 장애가 생기지 않아야 합니다.
6. 검증 방법도 함께 생각해 주세요.(자동화 테스트 코드, 실기기 테스트 등등)
7. Plan을 작성할 때 구체적인 코드 스니펫을 포함해 주세요.
...
PR이 머지되면 plan.md는 삭제하지 않고 **compact**한 뒤 `done/` 하위 폴더로 옮깁니다.
(예: `frontend/docs/artifacts/plan/2026-08-25_xxx_plan_.md` → `frontend/docs/artifacts/plan/done/2026-08-25_xxx_plan_.md`)

**기준**: 코드를 보면 알 수 있는 내용은 삭제하고, 코드를 봐도 복원 안 되는 내용만 남깁니다.

| 항목 | 처리 | 이유 |
| --- | --- | --- |
| Issue / PR 링크 | 유지 | 트래킹 목적 그 자체 |
| 요구사항 (원 요청/배경) | 유지 | 코드만 봐서는 "왜 이 기능이 필요했는지" 복원 불가 |
| 왜 이 방식을 택했는가 / 기각한 대안 | 유지 | 코드에 안 남는 정보 |
| 유즈케이스 표, 객체/메서드 스니펫 | 삭제 | 실제 구현 코드가 정답 |
| Phase 표 (일정, 파일경로) | 삭제 | 완료 후엔 의미 없음 |
| 검증 체크리스트, 아키텍처 검증 | 삭제 | 테스트 코드/리뷰가 대체 |
...
## 왜 이렇게 했는가
(기각한 대안, 제약조건 등 코드에 안 드러나는 것만)
```

남길 내용이 없으면(이슈/PR에 이미 배경이 다 있으면) 파일을 삭제해도 됩니다.
