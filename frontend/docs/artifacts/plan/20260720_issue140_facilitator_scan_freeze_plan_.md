...
1. `ios/Runner/Info.plist`에 `NSCameraUsageDescription` 추가 (확정된 원인 수정).
   Apple 공식 문서("Providing Usage Descriptions for Accessing Protected Resources")는
   해당 문자열이 왜 접근이 필요한지를 설명해야 한다고 명시한다. "카메라 접근 권한이
   필요합니다"처럼 권한명을 그대로 되풀이하는 문구는 실제 앱 심사(Guideline 5.1.1)에서
   반려 사유가 되므로, 실제 기능(조 배지 QR 스캔으로 코너 방문 기록)을 명시하는 문구로
   작성한다: "코너몬은 조 배지의 QR 코드를 스캔해 캠프 참가자의 코너 방문을 기록하기
   위해 카메라를 사용합니다".
